...
until $\text{max}_s |v_{\pi}^{n+1}(s) - v_{\pi}^n(s)| < \theta$ for some small $\theta$. This is called **iterative policy evaluation**, as it evaluates the value of the policy at each state.
...
2. Repeat until policy converges:
    ...
    2. Policy Improvement:
        1. For each $s$:
            1. $\pi'(s) = \text{argmax}_a \sum_{s', r} p(s', r|s, a)[r + \gamma v_{\pi}(s')]$
